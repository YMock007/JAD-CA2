...
public class UpdateBookingClient {
    ...
    private static final String CLEANIFY_API_URL = "http://localhost:8081/cleanify-ws/acceptBooking";

    public static boolean acceptBooking(UpdateBookingDTO updateBookingDTO) {
        Client client = ClientBuilder.newClient();
        WebTarget target = client.target(CLEANIFY_API_URL);
        Invocation.Builder invocationBuilder = target.request(MediaType.APPLICATION_JSON);
        System.out.println("Updating booking client");
