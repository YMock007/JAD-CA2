...
import service.CompleteBookingClient;
...
public class CompleteBookingServlet extends HttpServlet {
    ...
    protected void doPost(HttpServletRequest request, HttpServletResponse response) throws ServletException, IOException {
       ...
       System.out.print("In complete servlet");
        ...
        // ✅ Call Cleanify API
        boolean success = CompleteBookingClient.completeBooking(updateBookingDTO);

        if (success) {
            response.setStatus(HttpServletResponse.SC_OK);
            response.getWriter().write("Booking accepted successfully");
            response.sendRedirect("views/accepted-jobs/acceptedJobs.jsp");
        } else {
